...
export function TopBar({ runtime, quality, memoryPaused, drawer, onQuality, onSearch, onDrawer, onSettings, onToggleMemory }: {
    ...
    <div className="brand-group">
      ...
      <div className="popover-anchor">
        ...
        {statusOpen && <div className="status-popover popover-panel" role="dialog" aria-label="System status">
          ...
          <button type="button" className="text-button" onClick={onToggleMemory}>{memoryPaused ? "Resume memory" : "Pause memory extraction"}</button>
        </div>}
      </div>
    </div>
    <nav className="top-actions" aria-label="Application controls">
      <button type="button" className="search-button" onClick={onSearch}><Search size={16} /><span>Search memory</span><kbd><Command size={12} aria-hidden="true" />K</kbd></button>
      <div className="popover-anchor">
        <button type="button" className="mode-button" onClick={() => setQualityOpen((open) => !open)} aria-expanded={qualityOpen} aria-haspopup="listbox"><Sparkles size={15} /> <span>{PRESET_DETAILS[quality].label}</span><ChevronDown size={13} /></button>
        {qualityOpen && <div className="quality-popover popover-panel" role="listbox" aria-label="Response quality">
          ...
          {(Object.keys(PRESET_DETAILS) as QualityPreset[]).map((preset) => <button type="button" role="option" aria-selected={quality === preset} className={quality === preset ? "selected" : ""} key={preset} onClick={() => { onQuality(preset); setQualityOpen(false); }}>
      ...
      <IconButton label={drawer === "graph" ? "Close knowledge graph" : "Open knowledge graph"} className={drawer === "graph" ? "active" : ""} onClick={() => onDrawer(drawer === "graph" ? null : "graph")}><GitBranch size={18} /></IconButton>
