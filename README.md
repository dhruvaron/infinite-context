...
**Release status: NOT ACCEPTED.** The current source targets database schema 17. Checked-in benchmark and load artifacts are historical (the full-load artifact is schema 11), and the package-level typecheck, lint, contract, test, build, E2E, load, and evaluation gates have not been completed on this hardening revision. Recorded provider/API spend remains USD 0.00. See the [acceptance trace](docs/acceptance-trace.md) for the exact evidence boundary.
