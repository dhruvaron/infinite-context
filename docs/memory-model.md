...
3. **Atomic claims** — subject/predicate/value assertions linked to one or more exact sources, authority, confidence, observation time, validity interval, and freshness.
...
External facts have freshness classes. Expiration removes a claim from default current retrieval but retains it for historical inspection.
...
- Accept or reject a normalized confirmation proposal. Legacy proposals that predate exact parent/claim/evidence/route/content guards are reject-only and must be replaced by a fresh normalized proposal.
